...
import { GameState, HistoryItem, Gem, Player, Particle, PowerUpType } from './types';
import { QUIZ_DATA, HISTORY_ITEMS } from './constants';
...
const App: React.FC = () => {
  ...
  const [subState, setSubState] = useState<SubState>(SubState.NONE);
  const [totalPoints, setTotalPoints] = useState(0); 
  const [stage, setStage] = useState(0);
  const [gachaCount, setGachaCount] = useState(0);
  const [collection, setCollection] = useState<number[]>([]);
  const [currentQuizIndex, setCurrentQuizIndex] = useState(0);
  const [lastGachaItem, setLastGachaItem] = useState<HistoryItem | null>(null);
  ...
  const gameData = useRef<{
    player: Player & { trail: {x: number, y: number}[] };
    gems: Gem[];
    particles: Particle[];
    bgStars: BGStar[];
    lastGemTime: number;
  ...
  const createGem = (width: number, height: number, isInitial: boolean = false): Gem => {
  ...
  const initLevel = useCallback((width: number, height: number) => {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden text-white font-sans bg-slate-950 select-none touch-none">
      ...
      {/* UIオーバーレイ */}
      {gameState === GameState.PLAYING && subState === SubState.NONE && (
      ...
      {/* クイズモーダル */}
      {gameState === GameState.QUIZ && subState === SubState.NONE && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/90 backdrop-blur-md z-50 p-6">
          <div className="bg-white p-8 rounded-[35px] w-full max-w-lg shadow-2xl border-t-8 border-blue-500">
            ...
            <div className="grid grid-cols-1 gap-3">
              {QUIZ_DATA[currentQuizIndex].choices.map((choice, i) => (
                <button
                  key={i}
